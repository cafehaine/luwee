...
local function remove_from_table(tab, elm)
	for i=1, #tab do
		if tab[i] == elm then
			table.remove(tab, i)
			break
...
local function add_to_table(tab, elm)
	local found = false
	for i=1, #tab do
		if tab[i] == elm then
			found = true
...
-- recursive copy, no protection for recursive tables, does not copy key tables
...
local function strip_leading_zeros(str)
	return tonumber(str:match("0*(%d+)"))
end

local function read_ext(iter)
	iter:next()
	color = iter:next()..iter:next()..iter:next()..iter:next()..iter:next()
	return EXT_COLORS[strip_leading_zeros(color)+1]
end

local function read_std(iter)
	color = iter:next()..iter:next()
	return COLORS[strip_leading_zeros(color)]
end

local function read_attr_color_sequence(iter)
	local attributes = {}
	while iter:peek():match(ATTRIBUTES) do
		add_to_table(attributes, iter:next())
	end
...
local function decode_sequence(iter, colors, attributes)
	...
	if control == "\x19" then
		...
		-- bars
		elseif char == "b" then
			--TODO bar/input related stuff
			iter:next()
			iter:next()
		-- foreground
		elseif char == "F" then
			iter:next()
			local fore, attrs = read_attr_color_sequence(iter)
			new_colors[1] = fore
			for _,attr in ipairs(attrs) do
				add_to_table(new_attributes, attr)
			end
		--background
		elseif char == "B" then
			iter:next()
			local back = read_attr_color_sequence(iter)
			new_colors[2] = back
		-- any ?
		elseif char == "*" then
			iter:next()
		...
		-- options/ncurses stuff
		else
			--TODO use result
			-- (weird weechat and ncurses specific stuff)
			read_attr_color_sequence(iter)
		end
	-- add attr
	elseif control == "\x1A" then
		local attr = iter:next()
		add_to_table(new_attributes, attr)
	-- remove attr
	elseif control == "\x1B" then
		local attr = iter:next()
		remove_from_table(new_attributes, attr)
	-- reset
	elseif control == "\x1C" then
		new_colors = {nil, nil}
		new_attributes = {}
	end
	return {colors=new_colors, attributes=new_attributes}
end
